...
async function main() {
  const storage = new NFTStorage({ endpoint: new URL('https://api.nft.storage'), token: "fe73a53f.ed6732489e2447b5a902f9d77adbde62" })
  const metadata = await storage.store({
    ...
    properties: {
      ...
      file: new File(
        [await fs.promises.readFile('pinpie.jpg')],
        'pinpie.jpg',
        {
          type: 'image/jpg',
        }
